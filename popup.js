document.addEventListener("DOMContentLoaded", () => {
  ...
  themeToggle.onclick = () =>
    applyPopupTheme(!document.documentElement.classList.contains("dark"));
  ...
  const initContext = async () => {
    ...
    if (tabs.length > 0) {
      ...
      if (activeOrigin && activeOrigin.includes("http")) {
        originDisplay.innerText = activeOrigin
          .replace("https://", "")
          .replace("http://", "");
  ...
  const generatePills = (chunks, isNew) => {
    const extraClass = isNew
      ? "border-teal-500/50 bg-teal-50/50 dark:bg-teal-900/20"
      : "opacity-70";
    return chunks
      .toReversed()
      .map((c) => `<span class="chunk-pill ${extraClass}">${c}</span>`)
      .join("");
  };
  ...
  btnCopyNewEl.onclick = () => {
    ...
    if (btnCopyNewEl.resetTimeout) clearTimeout(btnCopyNewEl.resetTimeout);

    const originalHTML = btnCopyNewEl.innerHTML;
    navigator.clipboard
      .writeText([...currentNewChunks].sort().join(","))
      .then(() => {
        btnCopyNewEl.innerHTML = iconCheck;
        btnCopyNewEl.resetTimeout = setTimeout(() => {
          btnCopyNewEl.innerHTML = originalHTML;
        }, 1000);
      });
  };
  ...
  btnCopyHistoryEl.onclick = (e) => {
    ...
    if (btnCopyHistoryEl.resetTimeout) {
      clearTimeout(btnCopyHistoryEl.resetTimeout);
    }

    const originalHTML = btnCopyHistoryEl.innerHTML;
    navigator.clipboard
      .writeText([...currentHistoryChunks].sort().join(","))
      .then(() => {
        btnCopyHistoryEl.innerHTML = iconCheck;
        btnCopyHistoryEl.resetTimeout = setTimeout(() => {
          btnCopyHistoryEl.innerHTML = originalHTML;
        }, 1000);
      });
  };
  ...
  document.getElementById("clear").onclick = () => {
    chrome.runtime.sendMessage(
      { type: "CLEAR", payload: { tabId: activeTabId } },
      () => {
        sessionSeen = [];
        currentNewChunks = [];
  ...
  const renderSavedLists = () => {
    chrome.storage.local.get([GLOBAL_SAVED_KEY], (res) => {
      ...
      savedListContainerEl.innerHTML = lists
        .map((list) => {
          const originTag = list.origin
            ? `<span title=${list.origin.replace("https://", "")} class="text-[9px] px-1.5 py-[1px] bg-zinc-200 dark:bg-zinc-800 rounded text-zinc-500 max-w-[100px] truncate ml-2 shrink-0">${list.origin.replace("https://", "")}</span>`
            : "";

          return `
        <div class="border border-custom rounded-md p-2 flex flex-col bg-zinc-50 dark:bg-[#ffffff08]">
          <div class="flex items-center justify-between cursor-pointer action-toggle select-none" data-id="${list.id}">
      ...
      document.querySelectorAll(".action-toggle").forEach((header) => {
        header.onclick = () => {
          if (header.resetTimeout) {
            clearTimeout(header.resetTimeout);
          }
          ...
          header.resetTimeout = setTimeout(() => {
      ...
      document.querySelectorAll(".action-copy").forEach((btn) => {
        btn.onclick = (e) => {
          ...
          if (targetList) {
            navigator.clipboard
              .writeText(targetList.chunks.join(","))
              .then(() => {
                btn.innerHTML = iconCheck;
                setTimeout(() => {
